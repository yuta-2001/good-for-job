<x-app-layout>
	<x-slot name="header">
			<h2 class="font-semibold text-xl text-gray-800 leading-tight">
					企業情報編集<span class="ml-2 text-red-400 text-sm">*全て必須項目です。</span>
			</h2>
	</x-slot>
								...
								<section class="text-gray-600 body-font relative">
									<div class="container px-5 mx-auto">
										<div class="lg:w-4/5 md:w-2/3 mx-auto">
											...
											<form method="POST" action="{{ route('company.information.update', ['information' => Auth::id()]) }}" enctype="multipart/form-data">
												@csrf
												@method('PUT')
												<div class="-m-2">
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<input value="{{ $company->name }}" type="text" id="name" name="name" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															<label for="email" class="leading-7 text-sm text-gray-600 font-bold">メールアドレス</label>
															<input value="{{ $company->email }}" type="email" id="email" name="email" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<select id="industory" name="industory_id" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" aria-label="Default select example">
																	...
																	@foreach ($industories as $industory)
																		<option 
																			@if ($industory->id === $company->industory->id)
																				@selected(true)
																			@endif
																		value="{{ $industory->id }}">{{ $industory->name }}</option>
																	@endforeach
															</select>
														</div>
													</div>
														...
														<div class="relative">
															...
															<select id="prefecture" name="prefecture_id" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" aria-label="Default select example">
																	...
																	@foreach ($prefectures as $prefecture)
																		<option
																			@if($prefecture->id === $company->prefecture->id)
																				@selected(true)
																			@endif
													...
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<input value="{{ $company->address }}" type="text" id="address" name="address" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															<label for="president" class="leading-7 text-sm text-gray-600 font-bold">代表者名</label>
															<input value="{{ $company->president }}" type="text" id="president" name="president" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															<label for="count_of_employee" class="leading-7 text-sm text-gray-600 font-bold">従業員数</label>
															<input value="{{ $company->count_of_employee }}" type="text" id="count_of_employee" name="count_of_employee" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
													...
													<div class="p-2 w-1/2 mx-auto mb-4">
														<div class="relative">
															<div id="preview" class="h-auto">
																<img id="previewImage" src="{{ Storage::url($company->img) }}" alt="">
															</div>  
														</div>
													</div>
													<div class="p-2 w-full">
														<button type="button" onclick="location.href='{{ route('company.information.show', ['information' => Auth::id()]) }}'" class="flex mx-auto text-white bg-gray-400 border-0 py-2 px-8 focus:outline-none hover:bg-gray-500 rounded text-lg mb-3">戻る</button>
