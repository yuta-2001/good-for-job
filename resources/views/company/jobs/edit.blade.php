<x-app-layout>
	<x-slot name="header">
			<h2 class="font-semibold text-xl text-gray-800 leading-tight">
					求人編集<span class="ml-2 text-red-400 text-sm">*全て必須項目です。</span>
			</h2>
	</x-slot>
								...
								<section class="text-gray-600 body-font relative">
									<div class="container px-5 mx-auto">
										<div class="lg:w-4/5 md:w-2/3 mx-auto">
											...
											<form method="POST" action="{{ route('company.jobs.update', ['job' => $job->id]) }}" enctype="multipart/form-data">
												@csrf
												@method('PUT')
												<div class="-m-2">
															...
															<select id="occupation" name="occupation_id" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" aria-label="Default select example">
																...
																@foreach ($occupations as $occupation)
																	<option value="{{ $occupation->id }}"
																		@if ($occupation->id === $job->occupation_id)
																			@selected(true)
																		@endif
															...
															<select id="employment_type" name="employment_type_id" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" aria-label="Default select example">
																...
																@foreach ($employment_types as $type)
																	<option value="{{ $type->id }}"
																		@if ($type->id === $job->employment_type_id)
																			@selected(true)
																		@endif	
																	>{{ $type->name }}</option>
																@endforeach
													...
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<input type="file" id="img" name="img" onchange="imgPreView(event)" accept="image/png,image/jpeg,image/jpg" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto mb-4">
														<div class="relative">
															<div id="preview" class="h-auto">
																<img id="previewImage" src="{{ Storage::url($job->img) }}" alt="">
															</div>  
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
													...
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<input type="text" id="address" value="{{ $job->address }}" name="address" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															<label for="access" class="leading-7 text-sm text-gray-600 font-bold">アクセス</label>
															<textarea name="access" id="access" name="access" required class="h-40 resize-none w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $job->access }}</textarea>
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															<label for="payment" class="leading-7 text-sm text-gray-600 font-bold">給与</label>
															<textarea name="payment" id="payment" required class="h-40 resize-none w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $job->payment }}</textarea>
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															<label for="content" class="leading-7 text-sm text-gray-600 font-bold">仕事内容</label>
															<textarea name="content" id="content" required class="h-40 resize-none w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $job->content }}</textarea>
														</div>
													</div>
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<div class="form-check form-check-inline">
																<input class="form-check-input" name="feature_ids[]" type="checkbox" id="feature_{{ $feature->id }}" value="{{ $feature->id }}"
																	@foreach ($job->features as $job_feature)
																		@if ($job_feature->id === $feature->id)
																			@checked(true)				
																		@endif
																	@endforeach
																>
													...
													<div class="p-2 w-1/2 mx-auto">
														<div class="relative">
															...
															<div class="flex items-center pl-4 rounded border border-gray-200 dark:border-gray-700">
																<input id="bordered-checkbox-1" type="checkbox" value="1" name="status" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" required
																	@if ($job->status === 1)
																		@checked(true)
																	@endif
																>
